...
const ChatQueryTextField: React.FC<ChatQueryTextFieldProps> = ({
    ...
    return (
        <div className="flex flex-col justify-center items-center px-[108px] py-[24px]">
            <div className="flex w-full border  focus:border-Primary-color-4 py-[10px] px-[10px] rounded-full">
                <input
                    type="text"
                    value={query}
                    onChange={onChange}
                    placeholder={placeholder}
                    className="w-full  pr-5   focus:outline-none text-[16px] text-Neutral-color-2"
                />

                <button className='flex items-center content-center justify-center w-[45px] h-[45px] text-center bg-Primary-color-4 rounded-full'>
